...
# Impute NAs in gene-level variant count columns
...
param_grid = {
    'num_leaves': list(np.arange(10, 21, 2)),
    'lambda_l2': list(np.arange(0, 3, 1)),
    'max_bin': list(np.arange(100, 351, 50)),
    'min_data_in_leaf': list(np.arange(100, 1001, 100))
}
...
grid_search = GridSearchCV(
    ...
    refit='neg_root_mean_squared_error'  # Use RMSE for refitting and best_* attributes
)
...
print(f"Best parameters found: {best_params_safe}", flush=True)
print(f"Best cross-validation RMSE: {-grid_search.best_score_:.4f}", flush=True)
...
final_model.booster_.save_model('results/results_vars/lightGBM_results_vars/lightgbm_vars_tuned_model.txt')

# --- Performance Analysis ---
print("\n--- Performance Analysis ---", flush=True)

# Plot training and validation loss from the retrained model
...
ax.plot(evals_result['early_val']['rmse'], label='Validation RMSE')
ax.axvline(x=final_model.best_iteration_, color='r', linestyle='--', label=f'Best Iteration ({final_model.best_iteration_})')
ax.legend()
plt.ylabel('RMSE')
plt.xlabel('Training Iteration')
plt.title('LightGBM RMSE vs. Training Iteration (With Variants, Non-Normalized)')
plt.grid(True)
plot_filename = 'results/results_vars/lightGBM_results_vars/lightgbm_vars_tuned_loss_plot.png'
...
metrics_filename = 'results/results_vars/lightGBM_results_vars/lightgbm_vars_tuned_evaluation_metrics.txt'
...
importance_filename = 'results/results_vars/lightGBM_results_vars/lightgbm_vars_tuned_feature_importance.txt'
...
ax.set_title('LightGBM Top 20 Feature Importances (With Variants, Non-normalized)')
plt.tight_layout()
fi_plot_filename = 'results/results_vars/lightGBM_results_vars/lightgbm_vars_tuned_top20_feature_importance.png'
