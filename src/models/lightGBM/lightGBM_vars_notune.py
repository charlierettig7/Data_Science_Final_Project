...
# Extract label, extract groups to ensure no overlapping cell lines and drop metadata columns
y = data[label]
groups = data['cellosaurus_id']
x = data.drop(columns=[label, 'cellosaurus_id', 'sig_id', 'pert_id', 'inchi_key',
                       'cell_drug', 'cmap_name', 'smiles', 'cellosaurus_ids.accession'])
print(f"lengths -> x: {len(x)}, y: {len(y)}, groups: {len(groups)}", flush=True)
numeric_cols = x.select_dtypes(include=np.number).columns.tolist()
print(f"Identified numeric columns: {numeric_cols}", flush = True)
...
num_rounds = 500 # Increased rounds, will stop early if no improvement
...
bst.save_model('./results/results_vars/lightGBM_results_vars/lightgbm_vars_notune_model.txt')
...
plt.grid(True)
plot_filename = './results/results_vars/lightGBM_results_vars/lightgbm_vars_notune_loss_plot.png'
...
metrics_filename = './results/results_vars/lightGBM_results_vars/lightgbm_vars_notune_evaluation_metrics.txt'
...
importance_filename = './results/results_vars/lightGBM_results_vars/lightgbm_vars_notune_feature_importance.txt'
...
ax.set_title(f"LightGBM Top {top_n} Feature Importances (With Variants, Non-Normalized)")
plt.tight_layout()

feature_importance_plot_filename = './results/results_vars/lightGBM_results_vars/lightgbm_vars_notune_feature_importance_plot.png'
