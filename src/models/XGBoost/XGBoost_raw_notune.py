...
def load_and_preprocess(file_path_no_vars):
    ...
    # Extract groups to ensure no overlapping cell lines
    groups = x['cellosaurus_id']

    # Drop metadata columns before splitting
    x = x.drop(columns=['sig_id', 'pert_id', 'cellosaurus_id', 'inchi_key',
                       'cell_drug', 'cmap_name', 'smiles'])
    
    # Split data into train and temp (val+test) ensuring stratified cell lines
...
def train_model(dtrain, dval, params_path, num_rounds=500, early_stopping_rounds=10):
    ...
    bst.save_model('results/results_raw/XGBoost_results_raw/xgboost_model_raw_notune.json')
    return bst, evals_result


def analyze_performance(bst, dtest, y_test, evals_result):
    """
    ...
    plt.grid(True)
    plot_filename = 'results/results_raw/XGBoost_results_raw/xgboost_loss_plot_raw_notune.png'
    ...
    metrics_filename = 'results/results_raw/XGBoost_results_raw/xgboost_evaluation_metrics_raw_notune.json'
    ...
    importance_filename = 'results/results_raw/XGBoost_results_raw/xgboost_feature_importance_raw_notune.txt'
    ...
    ax.set_title(f"XGBoost Top {top_n} Feature Importances (Without Variants, Non-Normalized)")
    plt.tight_layout()
    feature_importance_plot_filename = 'results/results_raw/XGBoost_results_raw/xgboost_raw_notune_feature_importance_plot.png'
...
def main():
    ...
    dtrain, dval, dtest, y_test, feature_names = load_and_preprocess(file_path)
    print(f'number of training cols (raw): {dtrain.num_col()}')
    
    bst, evals_result = train_model(dtrain, dval, params_path)
    
    analyze_performance(bst, dtest, y_test, evals_result)
