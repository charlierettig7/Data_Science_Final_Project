...
def load_and_preprocess(file_path_no_vars, file_path_vars):
    ...
    Parameters:
        ...
        file_path_vars (str): Path to parquet file containing features with variants.
    ...
    # Extract label, groups, and features ensuring aligned sample counts
    y = data[label]
    groups = data['cellosaurus_id']
    x = data.drop(columns=[label, 'sig_id', 'pert_id', 'cellosaurus_id','inchi_key',
...
def train_model(dtrain, dval, params_path, num_rounds=500, early_stopping_rounds=10):
    ...
    bst.save_model('results/results_vars/XGBoost_results_vars/xgboost_model_vars_notune.json')
...
def analyze_performance(bst, dtest, y_test, evals_result, feature_names):
    ...
    plt.title('XGBoost RMSE vs. Training Iteration (With Variants, Non-Normalized)')
    plt.grid(True)
    plot_filename = 'results/results_vars/XGBoost_results_vars/xgboost_loss_plot_vars_notune.png'
    ...
    metrics_filename = 'results/results_vars/XGBoost_results_vars/xgboost_evaluation_metrics_vars_notune.json'
    ...
    importance_filename = 'results/results_vars/XGBoost_results_vars/xgboost_feature_importance_vars_notune.txt'
    ...
    ax.set_title(f"XGBoost Top {top_n} Feature Importances (With Variants, Non-Normalized)")
    plt.tight_layout()
    feature_importance_plot_filename = 'results/results_vars/XGBoost_results_vars/xgboost_vars_notune_feature_importance_plot.png'
...
def main():
    ...
    params_path = './src/models/XGBoost/xgboost_config_vars.json'

    dtrain, dval, dtest, y_test, feature_names = load_and_preprocess(file_path, file_path_vars)
    print(f'Number of training columns (vars): {dtrain.num_col()}')
