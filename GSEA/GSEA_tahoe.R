...
tahoe <- arrow::read_parquet("data/tahoe_no_overlapping_gene_level_matrix.parquet") |> 
  dplyr::select(responses, tidyselect::matches("^\\d+$", perl = TRUE)) |> 
  dplyr::mutate(index = dplyr::row_number())

files <- c("predictions_xgboost_raw.csv", "predictions_xgboost_vars.csv",
           "predictions_lgbm_raw.csv", "predictions_lgbm_vars.csv")
...
for(x in 1:4){
  ...
  prediction <- readr::read_csv(files[x]) |> 
    dplyr::right_join(tahoe, by = "index") |> 
  ...
  readr::write_csv(do.call(rbind, comparison_results), paste0(name[x], "/tahoe_GSEA_data.csv"))
}
