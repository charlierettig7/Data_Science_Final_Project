...
- **bash_scripts/**  
  Contains shell scripts relevant to project preprocessing and automation.

- **GSEA/**  
  Directory for Gene Set Enrichment Analysis scripts and zipped models.  
  - `GSEA.R` - Main R script for GSEA analysis  
  - `lgbm_raw.zip`, `lgbm_vars.zip`, `xgboost_raw.zip`, `xgboost_vars.zip` - Pretrained model files and variables  

- **Process_patient_data/**  
  Scripts, notebooks, data, and plotting outputs for processing patient-specific data.  
  - **data/**  
    Contains patient data files, intermediate output, mutation files, VCF files, and parquet files.  
    - `counts_patient5.csv`, `counts_patient6.csv` *(counts_patient6.csv is large, derived through process_patient_expression.R)*  
    - `gene_expression_processed_pat5.csv`, `gene_expression_processed_pat6.csv`  
    - `mutation_pat5.csv`, `mutation_pat6.csv`, `mutation_pat12.csv`  
    - `patient5.RDS`, `patient6.RDS`, `patient12.RDS`  
    - `pat5_to_predict.parquet`, `pat6_to_predict.parquet`  
    - `row_to_subclone_pat5.parquet`, `row_to_subclone_pat6.parquet`  
    - `SRR30720406_.vcf`, `SRR30720407_.vcf` *(large file from scAllele)*, `SRR30720408_.vcf`  
    - Other files such as `aggregated_muts_5.csv`, `.DS_Store`  
  - `filter_vcf.R`  
  - Patient pseudobulk notebooks: `pat_5_pseudobulk.ipynb`, `pat_6_pseudobulk.ipynb`, `pat_12_pseudobulk.ipynb`  
  - `process_patient_expression.R`  
  - **plots/**  
    Visualization outputs like UMAPs, elbow plots, PDF reports:  
    - `Rplots_6.pdf`, `Rplots_12.pdf`, `patient_5_Rplots.pdf`  
    - `patient5_UMAP.png`, `patient6_UMAP.png`, `patient12_UMAP.png`  
    - `patient5_elbowplot.png`, `patient6_elbowplot.png`, `patient12_elbowplot.png`  
  - `scTherapy_plus.R`  

- **Preprocess_Variants/**  
  Scripts and notebooks related to variant annotation and mutation data processing.  
  - `cell_annotate.R`  
  - `create_mutation_presence_cols_np.py`  
  - **data/**  
    - `Model.csv`  
    - `OmicsSomaticMutations.csv` *(large source file from depmap)*  
    - `mutations_cellosaurus_full.csv` *(large, derived via bash_scripts/annotate_cell_ids.sh)*  
...
**Note:**  
- Files marked as large are not included directly in the repository due to size limitations; they are either obtained externally or generated by preprocessing scripts.  
- `.DS_Store` files are macOS system files and can be ignored or removed.
